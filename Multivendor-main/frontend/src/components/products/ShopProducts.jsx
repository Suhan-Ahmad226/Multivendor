        ...
        <div className={`w-full grid ${styles === 'grid' ? 'lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-1' : 'grid-cols-1'} gap-6`}>
            {products.map((p, i) => {
                ...
                return (
                    <div 
                        key={i} 
                        className={`flex transition-all duration-300 hover:shadow-lg hover:-translate-y-1 ${styles === 'grid' ? 'flex-col' : 'flex-row items-center'} bg-white rounded-md overflow-hidden relative`}
                    >
                        {/* Product Image */}
                        ...
                        <div className={`flex flex-col justify-start ${styles==='grid' ? 'p-4 gap-2' : 'p-3 gap-1 w-1/2'}`}>
                            <h3 className="text-md font-semibold text-slate-700">{p?.name || "Unnamed Product"}</h3>
                            {p?.brand && <p className="text-sm text-slate-500">Brand: {p.brand}</p>}
                            <div className="flex items-center gap-2">
                                <span className="text-lg font-bold text-orange-500">${finalPrice}</span>
                                {discount > 0 && (
