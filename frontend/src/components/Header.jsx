...
const Header = ({ logoSrc = "/images/logo.png" }) => {
  ...
  // Mock product list for predictive search (you can use real product list from store)
  // If you maintain products in redux, replace this with that selector.
  const mockProducts = (categorys || [])
    .flatMap((c) => c.products || [])
    .slice(0, 200); // if categories contain products
  // fallback example suggestions if no products
  ...
  // Debounced predictive search (simple client-side)
  useEffect(() => {
    if (!searchValue) {
      setSearchSuggestions([]);
      return;
  ...
  const handleSearchSubmit = (e) => {
    ...
    navigate(`/products/search?category=${encodeURIComponent(
      categoryFilter || ""
  ...
  // small helper to render category image (fallback)
  ...
  return (
    <header className="w-full sticky top-0 z-50">
      ...
      <div
        className="hidden md:flex justify-between items-center bg-gradient-to-r from-blue-50 to-emerald-50 px-6 h-10 text-sm text-slate-700"
        aria-hidden={false}
      >
        <div className="flex gap-6 items-center">
          <div className="flex items-center gap-2" aria-label="support email">
            <MdEmail />
            <a href="mailto:support@example.com" className="underline">
              support@example.com
            </a>
          </div>
          <div className="flex items-center gap-2" aria-label="support phone">
            <IoMdPhonePortrait />
            <a href="tel:+1234323343" className="underline">
              +(123) 4323 343
            </a>
          </div>
        </div>
        ...
        <div className="flex items-center gap-6">
          ...
          {/* language selector */}
          <div className="relative" aria-haspopup="listbox">
            <button
              className="flex items-center gap-1 px-2 py-0.5 rounded hover:bg-white/30 focus:outline-none focus:ring-2 focus:ring-emerald-300"
          ...
          <div ref={accountRef} className="relative">
            {userInfo ? (
              ...
              <Link
                to="/login"
                className="flex items-center gap-2 px-2 py-1 rounded hover:bg-white/30"
                aria-label="Login"
              >
                <FaLock />
                <span>Login</span>
            ...
            {userInfo && showAccountDropdown && (
              <div
                ...
                className="absolute right-0 mt-2 w-48 bg-white border rounded shadow-lg p-2 text-sm"
              >
                <Link
                  to="/dashboard"
          ...
          <nav className="hidden md:flex items-center gap-4 font-semibold uppercase text-sm">
            <Link
              ...
              className={`px-2 py-1 rounded ${
                pathname === "/" ? "text-emerald-600" : "text-slate-600"
              }`}
            >
              Home
            </Link>
            ...
            <div className="relative" ref={megaRef}>
              <button
                className={`px-2 py-1 rounded flex items-center gap-2 ${
                  pathname.startsWith("/shops") ? "text-emerald-600" : "text-slate-600"
                }`}
                aria-haspopup="true"
              ...
              {showMegaMenu && (
                <div
                  onMouseEnter={() => setShowMegaMenu(true)}
                  onMouseLeave={() => setShowMegaMenu(false)}
                  className="absolute left-0 mt-2 w-[900px] bg-white border shadow-lg p-4 grid grid-cols-4 gap-4 z-40"
                  role="menu"
                >
                  {/* Categories grid */}
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {categorys.slice(0, 8).map((c, i) => (
                        <Link
                          ...
                          className="flex items-center gap-3 p-2 rounded hover:bg-emerald-50"
                        >
                  ...
                  <div className="col-span-2">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="bg-gradient-to-r from-emerald-50 to-emerald-100 rounded p-4 flex flex-col justify-between">
                        <div>
            ...
            <Link
              to="/deals"
              className={`px-2 py-1 rounded ${
                pathname === "/deals" ? "text-emerald-600" : "text-slate-600"
              }`}
            >
              Deals
            </Link>
            <Link
              to="/blog"
              className={`px-2 py-1 rounded ${
                pathname === "/blog" ? "text-emerald-600" : "text-slate-600"
              }`}
            >
              Blog
            </Link>
            <Link
              to="/about"
              className={`px-2 py-1 rounded ${
                pathname === "/about" ? "text-emerald-600" : "text-slate-600"
              }`}
            >
              About
            </Link>
            <Link
              to="/contact"
              className={`px-2 py-1 rounded ${
                pathname === "/contact" ? "text-emerald-600" : "text-slate-600"
              }`}
            >
              Contact
            </Link>
          ...
          <div className="w-full md:w-[520px] relative" ref={searchRef}>
            ...
            >
              <select
                ...
                className="hidden md:block px-2 border-r bg-transparent text-sm"
              ...
              <button
                type="submit"
                className="bg-emerald-600 text-white px-4 py-2 flex items-center gap-2"
                aria-label="Search"
              >
                <FaSearch />
            ...
            {searchSuggestions.length > 0 && (
              <ul
                className="absolute mt-1 w-full bg-white border rounded shadow-md z-50 max-h-60 overflow-auto"
                role="listbox"
              >
          ...
          <div ref={wishlistRef} className="relative">
            ...
            {showWishlistPreview && userInfo && (
              <div className="absolute right-0 mt-2 w-64 bg-white border rounded shadow-lg p-2 z-50">
                ...
                {/* Map wishlist items from store */}
                <div className="text-xs text-slate-500">You have {wishlist_count} items</div>
                <Link
                  to="/dashboard/my-wishlist"
                  className="block mt-3 text-emerald-600 text-sm"
                >
                  View wishlist
                </Link>
              </div>
      ...
      >
        ...
        <nav className="p-4 space-y-2">
          <Link
            to="/"
            onClick={() => setShowSidebar(false)}
            className="block py-2 px-2 rounded hover:bg-emerald-50"
          >
            Home
          </Link>
          <details className="group">
            <summary className="py-2 px-2 rounded hover:bg-emerald-50 cursor-pointer flex items-center justify-between">
              Categories <FaChevronRight />
            </summary>
          ...
          <div className="pt-4 border-t">
            {userInfo ? (
              ...
              <Link
                to="/login"
                onClick={() => setShowSidebar(false)}
                className="block mt-2 py-2 px-2 bg-emerald-600 text-white rounded text-center"
              >
                Login / Register
              </Link>
            )}
        ...
        <div className="absolute bottom-4 left-4 text-sm text-slate-600">
          ...
          <div className="mt-2 flex gap-2">
            ...
            <a href="#" aria-label="twitter"><FaExternalLinkAlt /></a>
          </div>
        </div>
      </aside>
      ...
      <nav className="fixed bottom-0 left-0 right-0 md:hidden bg-white border-t z-40">
        <div className="flex justify-between items-center max-w-xl mx-auto">
          <Link to="/" className="w-1/5 py-2 text-center text-sm">
            <div className="mx-auto">🏠</div>
            <div className="text-xs">Home</div>
          </Link>
          <button
            onClick={() => setShowSidebar(true)}
            className="w-1/5 py-2 text-center text-sm"
          ...
          <button
            onClick={() => navigate("/card")}
            className="w-1/5 py-2 text-center text-sm relative"
            aria-label="Cart"
          >
            <div>🛒</div>
            {card_product_count > 0 && (
