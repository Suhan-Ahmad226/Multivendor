...
import {
  ...
  FaAngleDown,
  FaBars,
  FaChevronRight, // <-- এই আইকনটি যুক্ত করা হয়েছে
  FaPhoneAlt,
  FaHome,
  FaList,
  FaShoppingBag,
        ...
        <div className="flex items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center gap-6 text-slate-600">
            ...
            <div ref={accountRef} className="relative group">
              ...
              {!userInfo && showAccountDropdown && (
                <div className="absolute right-0 mt-2 w-48 bg-white border rounded-lg shadow-lg p-3 text-sm animate-fade-in">
                  <Link
                    to="/login"
            ...
            <div className="relative group" ref={megaRef}>
              ...
              {showMegaMenu && (
                <div
                  onMouseEnter={() => setShowMegaMenu(true)}
                  onMouseLeave={() => setShowMegaMenu(false)}
                  //  START: Mega Menu Design Change
                  className="absolute left-0 mt-2 w-max max-w-5xl bg-white border rounded-lg shadow-xl p-6 grid grid-cols-5 gap-x-8 gap-y-6 z-40 animate-slide-down"
                  //  END: Mega Menu Design Change
                  role="menu"
                >
         {/* START: Categories grid with images */}
                  <div className="col-span-3">
                    <h4 className="text-md font-bold text-slate-800 mb-4">Shop by Category</h4>
                    <div className="grid grid-cols-2 gap-4">
                      {categorys.slice(0, 8).map((c, i) => (
                        <Link
                          key={i}
                          to={`/products?category=${encodeURIComponent(c.name)}`}
                          className="flex items-center gap-4 p-3 rounded-lg hover:bg-emerald-50 transition-colors duration-200 group"
                        ...
                        >
                          ...
                          <FaChevronRight className="text-slate-400 group-hover:text-emerald-600 transition-all" />
                        </Link>
                      ))}
                    </div>
                  </div>
                  {/* END: Categories grid with images */}
      ...
      >
        ...
        <nav className="p-4 space-y-2">
          ...
          <details className="group">
            ...
            {/* START: Mobile Category Menu Design Change */}
            <div className="pl-6 mt-2 space-y-1">
              {categorys.map((c, idx) => (
                <Link
                  ...
                  to={`/products?category=${encodeURIComponent(c.name)}`}
                  onClick={() => setShowSidebar(false)}
                  className="flex items-center justify-between py-2 px-3 text-sm text-slate-700 rounded hover:bg-slate-100 hover:text-emerald-600 transition-colors duration-200"
                >
                  <span>{c.name}</span>
             ...
             {/* END: Mobile Category Menu Design Change */}
          </details>
          <Link
            to="/deals"
            onClick={() => setShowSidebar(false)}
